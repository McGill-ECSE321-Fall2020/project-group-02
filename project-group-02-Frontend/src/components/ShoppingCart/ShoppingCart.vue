...
<template>
  <div id="app">
        ...
        <div class="row">
          ...
          <div class="col centering">
            <a
              href=""
              class="del"
              v-on:click.prevent="removeFromShoppingCart(item)"
              >Remove</a
            >
          </div>
          <div class="col centering">
    ...
    <router-link
      v-if="!$store.state.user.paymentCredentials.cvc"
      :to="{ name: 'edit' }"
    >
      <button
        v-if="!$store.state.user.paymentCredentials.cvc"
      ...
      while (i > 0) {
        this.AXIOS.post(
          "/".concat(this.$store.state.user.username) +
            "/shopping-cart/remove-item/".concat(item.name) +
            "/".concat(item.artist.username)
        )
          .then((response) => {
            console.log(
              "The item was successfully removed from the shopping cart."
            );
            i = -1;
          })
          .catch((error) => {});
